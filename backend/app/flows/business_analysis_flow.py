...
from app.research.evidence_gate import DEFAULT_MAX_RESEARCH_ATTEMPTS
...
class BusinessAnalysisRunNotFoundError(BusinessAnalysisFlowError):
    pass


class BusinessAnalysisRunStateError(BusinessAnalysisFlowError):
    pass


class BusinessAnalysisSnapshotError(BusinessAnalysisFlowError):
    pass
...
class BusinessAnalysisResearchStep:
    ...
    strategy_stage_run_id: UUID | None = None
...
class BusinessAnalysisFlow:
    ...
    def _validate_snapshot(
        ...
        try:
            ...
            raise BusinessAnalysisSnapshotError(
                "Analysis run contains an invalid profile snapshot"
            ) from exc

        readiness_result = evaluate_profile_readiness(
            profile_data=snapshot.profile_data,
            profile_metadata=snapshot.profile_metadata,
        ...
        if (
            ...
            raise BusinessAnalysisSnapshotError(
                "Analysis run snapshot is not ready for analysis"
            )
    ...
    def _ensure_initial_stage_runs(
        ...
        *,
        ...
        statement = select(AnalysisStageRun).where(
            AnalysisStageRun.analysis_run_id == analysis_run.id,
            AnalysisStageRun.attempt == 1,
        )
        existing_stage_runs = list(db.scalars(statement).all())
        existing_stages = {
            stage_run.stage
            for stage_run in existing_stage_runs
        }
    ...
    def initialize(
        ...
        *,
        ...
        if analysis_run.status not in allowed_statuses:
            raise BusinessAnalysisRunStateError(
                "Analysis run cannot be initialized from status "
                f"{analysis_run.status}"
            )

        self._validate_snapshot(analysis_run)
        self._ensure_initial_stage_runs(
            db=db,
            analysis_run=analysis_run,
        )

        if analysis_run.status == AnalysisRunStatus.QUEUED.value:
    ...
    def advance_research(
        ...
        *,
        ...
        if evaluation.gate.retry_stages:
            scheduled_retries = schedule_targeted_retries(
                db=db,
                evaluation=evaluation,
                max_attempts=max_attempts,
            )
        elif evaluation.gate.can_proceed:
            strategy_stage_run = self._ensure_business_strategy_stage_run(
                db=db,
                analysis_run_id=run_id,
            )
            strategy_stage_run_id = strategy_stage_run.id

        return BusinessAnalysisResearchStep(
            evaluation=evaluation,
            scheduled_retries=scheduled_retries,
            strategy_stage_run_id=strategy_stage_run_id,
        )
    ...
    def _ensure_business_strategy_stage_run(
        ...
        *,
        ...
        if analysis_run.status != AnalysisRunStatus.RUNNING.value:
            raise BusinessAnalysisRunStateError(
                "Business Strategy can only be scheduled for a RUNNING AnalysisRun"
            )

        statement = select(AnalysisStageRun).where(
            AnalysisStageRun.analysis_run_id == analysis_run_id,
            AnalysisStageRun.stage == AnalysisStage.BUSINESS_STRATEGY.value,
            AnalysisStageRun.attempt == 1,
        )
        ...
        strategy_stage_run = AnalysisStageRun(
            analysis_run_id=analysis_run_id,
            stage=AnalysisStage.BUSINESS_STRATEGY.value,
            attempt=1,
            status=AnalysisStageStatus.PENDING.value,
        )
        db.add(strategy_stage_run)
        db.flush()
    ...
    def _require_completed_strategy_result(
        ...
        *,
        ...
        strategy_stage_run = db.scalar(stage_statement)
        if strategy_stage_run is None:
            raise BusinessAnalysisRunStateError(
                "Finance cannot be scheduled before Business Strategy is COMPLETED"
            )

        result_statement = select(AnalysisResult).where(
            AnalysisResult.analysis_run_id == analysis_run_id,
            AnalysisResult.stage_run_id == strategy_stage_run.id,
            AnalysisResult.stage == AnalysisStage.BUSINESS_STRATEGY.value,
        )
        strategy_result = db.scalar(result_statement)
        if strategy_result is None:
            raise BusinessAnalysisRunStateError(
                "Completed Business Strategy has no persisted result"
            )
    ...
    def _ensure_finance_stage_run(
        ...
        *,
        ...
        if analysis_run.status != AnalysisRunStatus.RUNNING.value:
            raise BusinessAnalysisRunStateError(
                "Finance can only be scheduled for a RUNNING AnalysisRun"
            )
    ...
    def advance_strategy(
        ...
        *,
        ...
        if analysis_run.status != AnalysisRunStatus.RUNNING.value:
            raise BusinessAnalysisRunStateError(
                "Business Strategy can only advance while AnalysisRun is RUNNING"
            )
    ...
    def _require_completed_finance_result(
        self,
        *,
        db: Session,
        analysis_run_id: UUID,
        ...
        finance_stage_run = db.scalar(stage_statement)
        if finance_stage_run is None:
            raise BusinessAnalysisRunStateError(
                "Decision Analytics cannot be scheduled before Finance is COMPLETED"
            )

        result_statement = select(AnalysisResult).where(
            AnalysisResult.analysis_run_id == analysis_run_id,
            AnalysisResult.stage_run_id == finance_stage_run.id,
            AnalysisResult.stage == AnalysisStage.FINANCE.value,
        )
        finance_result = db.scalar(result_statement)
        if finance_result is None:
            raise BusinessAnalysisRunStateError(
                "Completed Finance stage has no persisted result"
            )

        return finance_result

    def _ensure_decision_analytics_stage_run(
        self,
        *,
        db: Session,
        analysis_run_id: UUID,
        ...
        if analysis_run.status != AnalysisRunStatus.RUNNING.value:
            raise BusinessAnalysisRunStateError(
                "Decision Analytics can only be scheduled for a RUNNING AnalysisRun"
            )
        ...
        analytics_stage_run = AnalysisStageRun(
            analysis_run_id=analysis_run_id,
            stage=AnalysisStage.DECISION_ANALYTICS.value,
            attempt=1,
            status=AnalysisStageStatus.PENDING.value,
        )
        db.add(analytics_stage_run)
        db.flush()
        return analytics_stage_run
    ...
    def advance_finance(
        ...
        *,
        ...
        if analysis_run.status != AnalysisRunStatus.RUNNING.value:
            raise BusinessAnalysisRunStateError(
                "Finance can only advance while AnalysisRun is RUNNING"
            )

        self._require_completed_finance_result(
            db=db,
            analysis_run_id=run_id,
        )
        return self._ensure_decision_analytics_stage_run(
            db=db,
            analysis_run_id=run_id,
        )
