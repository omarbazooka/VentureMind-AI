...
class FinanceAssumptionBuilderContext(BaseModel):
    ...
    def validate_context(
        ...
        if not self.research_gate.can_proceed:
            raise ValueError(
                "Finance assumption building requires a research "
                "gate that allows downstream progression"
            )
        ...
        for stage, result in results_by_stage.items():
            if (
                result is None
                and stage not in insufficient_stages
            ):
                raise ValueError(
                    "Finance assumption context is missing "
                    f"accepted research for {stage.value}"
                )

        return self


class FinancialAssumptionDraft(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    def validate_draft(
        ...
        if invalid_stages:
            raise ValueError(
                "Finance draft may only reference research stages"
            )

        if self.input_name in MONETARY_INPUTS:
            if has_value and self.currency is None:
                raise ValueError(
                    "Known monetary values require currency"
                )
        elif self.currency is not None:
            raise ValueError(
                "Non-monetary inputs cannot declare currency"
            )

        if self.input_name in UNIT_BASED_INPUTS:
            if has_value and not self.unit_label:
                raise ValueError(
                    "Known unit-based values require unit_label"
                )
        elif self.unit_label is not None:
            raise ValueError(
                "This input cannot declare unit_label"
            )

        if self.input_name in PERIOD_BASED_INPUTS:
            if has_value and self.period is None:
                raise ValueError(
                    "Known period-based values require a period"
                )
        elif self.period is not None:
            raise ValueError(
                "This input cannot declare a period"
            )

        if not has_value:
            if self.provenance is not None:
                raise ValueError(
                    "Unknown values cannot declare provenance"
                )
            if (
                self.profile_fields
                ...
                raise ValueError(
                    "Unknown values cannot claim source lineage"
                )
            return self

        if self.provenance is None:
            raise ValueError(
                "Known draft values require explicit provenance"
            )

        if self.provenance == FinancialAssumptionProvenance.USER:
            if not self.profile_fields:
                raise ValueError(
                    "USER draft values require profile field references"
                )
            if self.supporting_stages or self.evidence_source_ids:
                raise ValueError(
                    "USER values cannot claim research provenance"
                )
            ...
            if (
                ...
                raise ValueError(
                    "WEB values require research stages and "
                    "evidence source IDs"
                )
            if self.profile_fields:
                raise ValueError(
                    "WEB values cannot claim profile provenance"
                )
            ...
            if (
                ...
                raise ValueError(
                    "AI_ASSUMPTION must not claim external lineage"
                )

        return self


class FinancialScenarioAssumptionDraft(BaseModel):
    model_config = ConfigDict(extra="forbid")

    scenario: FinancialScenarioKind
    selling_price_per_unit: FinancialAssumptionDraft
    sales_volume: FinancialAssumptionDraft
    variable_cost_per_unit: FinancialAssumptionDraft
    ...
    def validate_slots(
        ...
        for field_name, input_name in expected.items():
            assumption = getattr(self, field_name)
            if assumption.input_name != input_name:
                raise ValueError(
                    f"{field_name} must use {input_name.value}"
                )
        ...
        if (
            ...
            raise ValueError(
                "starting_cash draft must use STARTING_CASH"
            )

        return self


class FinancialAssumptionDraftBundle(BaseModel):
    model_config = ConfigDict(extra="forbid")

    base: FinancialScenarioAssumptionDraft
    upside: FinancialScenarioAssumptionDraft
    downside: FinancialScenarioAssumptionDraft
    ...
    def validate_roles(
        ...
        if self.downside.scenario != FinancialScenarioKind.DOWNSIDE:
            raise ValueError(
                "downside draft must use DOWNSIDE"
            )
        return self
