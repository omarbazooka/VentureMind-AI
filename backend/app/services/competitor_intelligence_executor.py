from collections.abc import Callable
from uuid import UUID

from sqlalchemy.orm import Session

from app.models.analysis_result import (
    AnalysisResult,
)
from app.research.competitor_evidence import (
    CompetitorEvidenceVerificationError,
)
from app.schemas.analysis import (
    AnalysisStage,
)
from app.schemas.research import (
    CompetitorAnalysis,
    ResearchStageClaim,
)
from app.services.research_stage import (
    ResearchStageResultValidationError,
    claim_research_stage,
    complete_research_stage,
    fail_research_stage,
)


SessionFactory = Callable[
    [],
    Session,
]

CompetitorIntelligenceRunner = Callable[
    [ResearchStageClaim],
    CompetitorAnalysis,
]


class CompetitorIntelligenceExecutionError(
    RuntimeError
):
    pass


def _mark_competitor_intelligence_failed(
    *,
    session_factory: SessionFactory,
    stage_run_id: UUID,
    error_code: str,
    error_message: str,
) -> None:
    with session_factory() as db:
        fail_research_stage(
            db=db,
            stage_run_id=stage_run_id,
            error_code=error_code,
            error_message=error_message,
        )

        db.commit()


def execute_competitor_intelligence_stage(
    *,
    session_factory: SessionFactory,
    stage_run_id: UUID,
    runner: CompetitorIntelligenceRunner,
) -> AnalysisResult:
    with session_factory() as db:
        claim = claim_research_stage(
            db=db,
            stage_run_id=stage_run_id,
        )

        if (
            claim.stage
            != (
                AnalysisStage
                .COMPETITOR_INTELLIGENCE
            )
        ):
            raise (
                CompetitorIntelligenceExecutionError(
                    "Competitor intelligence "
                    "executor received a "
                    "non-competitor stage"
                )
            )

        db.commit()

    try:
        research_result = runner(
            claim
        )

    except (
        CompetitorEvidenceVerificationError
    ) as exc:
        _mark_competitor_intelligence_failed(
            session_factory=session_factory,
            stage_run_id=stage_run_id,
            error_code=(
                "INVALID_COMPETITOR_"
                "INTELLIGENCE_EVIDENCE"
            ),
            error_message=(
                "Competitor intelligence "
                "failed deterministic "
                "evidence verification."
            ),
        )

        raise (
            CompetitorIntelligenceExecutionError(
                "Competitor intelligence "
                "evidence verification failed"
            )
        ) from exc

    except Exception as exc:
        _mark_competitor_intelligence_failed(
            session_factory=session_factory,
            stage_run_id=stage_run_id,
            error_code=(
                "COMPETITOR_INTELLIGENCE_"
                "EXECUTION_ERROR"
            ),
            error_message=(
                "Competitor intelligence "
                "runner raised "
                f"{type(exc).__name__}."
            ),
        )

        raise (
            CompetitorIntelligenceExecutionError(
                "Competitor intelligence "
                "execution failed"
            )
        ) from exc

    try:
        with session_factory() as db:
            persisted_result = (
                complete_research_stage(
                    db=db,
                    stage_run_id=(
                        stage_run_id
                    ),
                    result_data=(
                        research_result
                    ),
                )
            )

            db.commit()

            return persisted_result

    except (
        ResearchStageResultValidationError
    ) as exc:
        _mark_competitor_intelligence_failed(
            session_factory=session_factory,
            stage_run_id=stage_run_id,
            error_code=(
                "INVALID_COMPETITOR_"
                "INTELLIGENCE_RESULT"
            ),
            error_message=(
                "Competitor intelligence "
                "returned an invalid "
                "structured result."
            ),
        )

        raise (
            CompetitorIntelligenceExecutionError(
                "Competitor intelligence "
                "returned an invalid "
                "structured result"
            )
        ) from exc